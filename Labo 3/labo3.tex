...
\begin{figure}[h!]
	...
	\begin{center}
		...
		\subfloat[1N5392]{\href{http://www.protostack.com/download/1N5391-9.pdf}{\includegraphics[width=2.7cm]{figures/QR_1N5392.pdf}}}\hspace{0.5cm}
...
{
Partie plateau suivie par une droite décroissante.

La première partie correspond à une grosse charge, un faible courant $I_{out}$ et une diode Zener en avalanche. La charge est donc régulée.

La partie décroissante correspond à un comportement en diviseur résistif lorsque la charge est trop importante et que la diode devient bloquante.
Déterminons l'équation de cette droite~: $V_{out} = V_2 \cdot \frac{R_{ch}}{R + R_{ch}}$ et $V_{out} = R_{ch} \cdot I_{out}$. Ainsi, $R_{ch} = \frac{V_{out}}{I_{out}}$, et donc $V_{out} = V_2 \cdot \frac{V_{out}/I_{out}}{R+ V_{out}/I_{out}} = V_2 \cdot \frac{V_{out}}{R\cdot I_{out} + V_{out}} \Leftrightarrow V_2 = R\cdot I_{out} + V_{out} \Leftrightarrow V_{out} = V_2 - R\cdot I_{out}$.
%TODO Add a graph of that stuff.
}%R
	\label{Q:24}
...
% \vspace{-1cm}
% 	\begin{center}
% 	\subfloat[Redresseur à mercure, BEAMS -- Laboratoire~d'électricité~bâtiment~L]{\includegraphics[width=4cm]{figures/IMG_3843rs2.jpg}}\hspace{1cm}
% 	\subfloat[Redresseur au Sélénium {\tiny \textit{transferred to fr.wikipedia Commons by User : Bloody-libu Public domain via Wikimedia Commons}}]{\includegraphics[width=4cm]{figures/Redresseur_Seleniums.jpg} 
% 	}\hspace{1cm}
% 	\subfloat[Diode~à~vide, BEAMS -- Laboratoire~d'électronique bâtiment~U]{\includegraphics[width=3cm]{figures/diode_viders.jpg}}
